...
class Spider(scrapy.Spider):
    ...
    def start_requests(self):
        for part, link in parts.items():
            path = 'results/{}.xlsx'.format(part)
            if not os.path.exists(path):
                for i in range(1, 11):  # 爬前十页
                    yield scrapy.Request('{}-{}'.format(link, i), meta={'part': part})
            else:
                print(part, '文件已存在')
    ...
    def parse(self, response):
        ...
        for li in response.xpath('//ul[@class="for-list"]/li'):
            item = ProjectItem()
            item['part'] = part
            title = li.xpath('div[@class="titlelink box"]/a[@class="truetit"]//text()').get()
            if title:
                ...
                if reply['content'].strip() and reply['is_ban'] == 0:  # 有些评论会被禁止,不显示
                    ...
                    if reply['quote']:  # 有引用
                        try:
                            ...
                            html.fromstring(reply['quote'][0]['header'][0].strip()).xpath(
                                '//a//text()')[0]
                        except:
                            item['comment_quote_author'] = ''
                        try:
                            item['comment_quote_content'] = ' '.join(
                                [text.strip() for text in
                                 html.fromstring(comment_quote_content=reply['quote'][0]['content'].strip()).xpath(
                                     '//text()')])
                        except:
